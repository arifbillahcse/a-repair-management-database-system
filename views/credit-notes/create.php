...
.item-net{width:17%;min-width:100px}
.item-rm{width:32px}
                    ...
                    <div class="form-grid-2">
                        <div class="form-group">
                            ...
                            <input type="number" id="cn_number" name="cn_number" class="form-input <?= isset($errors['cn_number']) ? 'is-invalid' : '' ?>"
                                   value="<?= Utils::e($fd['cn_number'] ?? $nextNumber) ?>" min="1" required>
                            <?php if (isset($errors['cn_number'])): ?><div class="invalid-feedback"><?= Utils::e($errors['cn_number']) ?></div><?php endif; ?>
                        </div>
                        <div class="form-group">
                            ...
                            <input type="date" id="cn_date" name="cn_date" class="form-input <?= isset($errors['cn_date']) ? 'is-invalid' : '' ?>"
                                   value="<?= Utils::e($fd['cn_date'] ?? date('Y-m-d')) ?>" required>
                            <?php if (isset($errors['cn_date'])): ?><div class="invalid-feedback"><?= Utils::e($errors['cn_date']) ?></div><?php endif; ?>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" id="customer_name" name="customer_name" class="form-input <?= isset($errors['customer_name']) ? 'is-invalid' : '' ?>"
                               value="<?= Utils::e($fd['customer_name'] ?? '') ?>" maxlength="200" placeholder="Full name or company" required>
                        <?php if (isset($errors['customer_name'])): ?><div class="invalid-feedback"><?= Utils::e($errors['customer_name']) ?></div><?php endif; ?>
                    </div>
                    <div class="form-group">
                        ...
                        <input type="text" id="customer_address" name="customer_address" class="form-input"
                               value="<?= Utils::e($fd['customer_address'] ?? '') ?>" maxlength="500" placeholder="Street, City, Country">
                    </div>
                    <div class="form-group" style="margin-bottom:0">
                        <label class="form-label" for="customer_vat">VAT N°</label>
                        <input type="text" id="customer_vat" name="customer_vat" class="form-input"
                               value="<?= Utils::e($fd['customer_vat'] ?? '') ?>" maxlength="50" placeholder="e.g. IT04601010269">
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body" style="padding:0;overflow-x:auto">
                    <table class="items-table" id="itemsTable">
                        ...
                        <tbody id="itemsBody">
                            <?php
                            $existingItems = $fd['items'] ?? [['description'=>'','basic_amount'=>'','vat_amount'=>'','net_amount'=>'']];
                            foreach ($existingItems as $idx => $item):
                            ?>
                            <tr class="item-row">
                                <td class="item-desc">
                                    <input type="text" name="items[<?= $idx ?>][description]" class="form-input item-desc-inp"
                                           value="<?= Utils::e($item['description'] ?? '') ?>" placeholder="Description of credit…">
                                </td>
                                <td class="item-num">
                                    <input type="number" name="items[<?= $idx ?>][basic_amount]" class="form-input item-basic num-inp"
                                           value="<?= Utils::e($item['basic_amount'] ?? '') ?>" step="0.01" min="0" placeholder="0.00">
                                </td>
                                <td class="item-num">
                                    <input type="number" name="items[<?= $idx ?>][vat_amount]" class="form-input item-vat num-inp"
...
(function () {
    ...
    addBtn.addEventListener('click', function () {
        ...
        tr.innerHTML =
            '<td class="item-desc"><input type="text" name="items[' + idx + '][description]" class="form-input item-desc-inp" placeholder="Description of credit…"></td>' +
            '<td class="item-num"><input type="number" name="items[' + idx + '][basic_amount]" class="form-input item-basic num-inp" step="0.01" min="0" placeholder="0.00"></td>' +
            '<td class="item-num"><input type="number" name="items[' + idx + '][vat_amount]" class="form-input item-vat num-inp" step="0.01" placeholder="0.00"></td>' +
