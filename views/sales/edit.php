...
$dateVal   = $fd['sale_date']     ?? $sale['sale_date'];
$custName  = $fd['customer_name'] ?? $sale['customer_name'];
$custIdVal = (int)($fd['customer_id'] ?? $sale['customer_id'] ?? 0);
$items     = $fd['items'] ?? [];
?>
<style>
...
<div class="page-header">
    <div>
        <h1 class="page-title">Edit Sale <?= Utils::e($sale['sale_number']) ?></h1>
        <p class="page-subtitle">Editing items returns and re-deducts stock automatically</p>
    </div>
    <a href="<?= BASE_URL ?>/sales/<?= (int)$sale['sale_id'] ?>" class="btn btn-secondary">
...
<form method="POST" action="<?= BASE_URL ?>/sales/<?= (int)$sale['sale_id'] ?>" id="saleForm">
    <input type="hidden" name="csrf_token"  value="<?= Utils::e($csrfToken) ?>">
    <input type="hidden" name="customer_id" id="customerId" value="<?= $custIdVal ?: '' ?>">
                ...
                <div class="card-body">
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
                        <div class="form-group" style="margin-bottom:0">
                            ...
                            <input type="text" class="form-input" value="<?= Utils::e($sale['sale_number']) ?>" readonly
                    ...
                    <div class="form-group" style="margin-top:1rem;margin-bottom:0">
                        ...
                        <div class="ac-wrap">
                            <input type="text" id="custSearch" name="customer_name" class="form-input" autocomplete="off"
                                   ...
                                   value="<?= Utils::e($custName) ?>">
                            <div class="ac-list" id="acList"></div>
                        </div>
                        <p style="font-size:.72rem;color:var(--text-muted);margin-top:.35rem" id="custHint">
                            <?= $custIdVal ? 'Linked to client profile ✓' : 'Pick from the list to link the sale to a client profile, or type any name for a walk-in sale.' ?>
                        </p>
                ...
                <div class="table-responsive">
                    <table class="items-table" id="itemsTable">
                        ...
                        <tbody id="itemsBody">
                            ...
                            <tr class="item-row">
                                <td>
                                    <select name="items[<?= $i ?>][product_id]" class="form-input item-prod-select item-prod">
                                        <option value="">— Custom item —</option>
                                        <?php foreach ($products as $p):
                                            $pStock = (int)$p['quantity_on_hand'];
                                            $isSel  = $pid === (int)$p['product_id'];
                                        ?>
                                        <option value="<?= (int)$p['product_id'] ?>"
                                            data-price="<?= (float)$p['selling_price'] ?>"
                                            data-stock="<?= $pStock ?>"
                                            <?= $isSel ? 'selected' : '' ?>
                                            <?= ($pStock === 0 && !$isSel) ? 'disabled' : '' ?>>
                                            <?= Utils::e($p['name']) ?><?= $p['sku'] ? ' [' . Utils::e($p['sku']) . ']' : '' ?> (<?= $pStock ?>)
                                        </option>
                                        <?php endforeach; ?>
                                    </select>
                                    <div class="stock-hint" data-stock-hint></div>
                                </td>
                                <td><input type="text" name="items[<?= $i ?>][description]" class="form-input item-desc-input" value="<?= Utils::e($item['description'] ?? '') ?>" placeholder="Description…" required></td>
                                <td><input type="number" name="items[<?= $i ?>][quantity]" class="form-input item-num-input item-qty" value="<?= $qty ?>" step="1" min="1"></td>
                                <td><input type="number" name="items[<?= $i ?>][unit_price]" class="form-input item-num-input item-price" value="<?= $price ?>" step="0.01" min="0"></td>
                                <td><input type="number" name="items[<?= $i ?>][discount_pct]" class="form-input item-small-input item-disc" value="<?= $disc ?>" step="0.1" min="0" max="100"></td>
                                <td class="line-total-cell" data-line-total><?= Utils::formatCurrency($lineTotal) ?></td>
        ...
        <div style="display:flex;flex-direction:column;gap:1.25rem">
            ...
            <div class="card">
                ...
                <div class="card-body" style="display:flex;flex-direction:column;gap:1rem">
                    ...
                    <?php $signatures = $signatures ?? []; $selSig = (int)($fd['signature_id'] ?? $sale['signature_id'] ?? 0); ?>
                    <div class="form-group" style="margin-bottom:0">
                        ...
                        <select id="signature_id" name="signature_id" class="form-input">
                            ...
                            <option value="1" <?= $selSig === 1 ? 'selected' : '' ?>><?= Utils::e($signatures['signature1']) ?></option>
                            <?php endif; ?>
                            <?php if (!empty($signatures['signature2'])): ?>
                            <option value="2" <?= $selSig === 2 ? 'selected' : '' ?>><?= Utils::e($signatures['signature2']) ?></option>
                            <?php endif; ?>
                            <?php if (!empty($signatures['signature3'])): ?>
                            <option value="3" <?= $selSig === 3 ? 'selected' : '' ?>><?= Utils::e($signatures['signature3']) ?></option>
                            <?php endif; ?>
                        </select>
                    </div>
                </div>
            ...
            <p style="font-size:.78rem;color:var(--text-muted);margin:0">
                Payment status and amount paid are managed from the sale's detail page, not here.
            </p>
            ...
            <div style="display:flex;gap:.75rem;flex-direction:column">
                ...
                <a href="<?= BASE_URL ?>/sales/<?= (int)$sale['sale_id'] ?>" class="btn btn-secondary" style="justify-content:center">Cancel</a>
            </div>
...
(function () {
    ...
    function productOptions(selectedId) {
        ...
        PRODUCTS.forEach(function (p) {
            html += '<option value="' + p.id + '" data-price="' + p.price + '" data-stock="' + p.stock + '"' +
                ...
                (p.stock === 0 && selectedId !== p.id ? ' disabled' : '') + '>' +
    ...
    // Attach listeners to server-rendered rows and compute initial totals
    document.querySelectorAll('#itemsBody .item-row').forEach(attachRow);
    if (!document.querySelectorAll('#itemsBody .item-row').length) addRow();
    recalc();
    ...
    toggleNewCustPanel(); // reflect state on load (e.g. sale already linked to a profile)

    custInput.addEventListener('input', function () {
        custId.value = '';                       // typing breaks the link
        document.getElementById('custHint').textContent =
