...
<div class="card" style="max-width:600px">
    <form method="POST" action="<?= BASE_URL ?>/personal-notes/<?= $note['note_id'] ?>">
        ...
        <div class="card-body" style="display:flex;flex-direction:column;gap:1.5rem">
            <div class="form-group">
                ...
                <input type="text" id="title" name="title" class="form-input <?= isset($errors['title']) ? 'is-invalid' : '' ?>"
                       value="<?= Utils::e($fd['title'] ?? $note['title']) ?>" maxlength="255" placeholder="Note title…" required autofocus>
            ...
            <div class="form-group">
                ...
                <textarea id="description" name="description" class="form-input <?= isset($errors['description']) ? 'is-invalid' : '' ?>"
                          rows="8" placeholder="Write your note…" required><?= Utils::e($fd['description'] ?? $note['description']) ?></textarea>
                <?php if (isset($errors['description'])): ?><div class="invalid-feedback"><?= Utils::e($errors['description']) ?></div><?php endif; ?>
            </div>

            <div style="padding:1rem;background:var(--bg-secondary);border-radius:var(--radius);font-size:0.875rem;color:var(--text-secondary)">
